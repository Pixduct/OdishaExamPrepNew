...
export const ActiveExamContextBar: React.FC<ActiveExamContextBarProps> = ({
  ...
  return (
    <>
      ...
      <DynamicVectorCard glowColor="rgba(37, 99, 235, 0.20)" className={`w-full mb-6 sm:mb-8 ${className}`}>
        <div className="w-full bg-gradient-to-r from-white via-blue-50/50 to-indigo-50/40 dark:bg-gradient-to-br dark:from-[#0B1528] dark:via-[#112240] dark:to-[#0B1528] text-slate-900 dark:text-white border border-slate-200/90 dark:border-blue-500/35 rounded-2xl sm:rounded-[2rem] p-3.5 sm:p-5 shadow-lg shadow-blue-500/5 dark:shadow-2xl dark:shadow-slate-950/60 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between gap-3 min-w-0 relative z-10">
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl flex items-center justify-center shrink-0 shadow-md ${
                isAll
                  ? 'bg-brand-50 text-brand-600 border border-brand-200/80 dark:bg-blue-500/20 dark:text-blue-300 dark:border-blue-400/40'
                  : 'bg-emerald-50 text-emerald-700 border border-emerald-200/80 dark:bg-emerald-500/20 dark:text-emerald-300 dark:border-emerald-400/40'
              }`}>
                {isAll ? (
                  <Layers className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-brand-600 dark:text-blue-300" />
                ) : (
                  <Target className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-emerald-600 dark:text-emerald-300" />
                )}
              </div>
