...
- **Dynamic Bi-Directional 3D Vector Card Hover (`DynamicVectorCard.tsx`)**: Real-time cursor tracking engine (`onMouseMove`) calculating relative card coordinates to dynamically tilt cards bi-directionally (tilts left when mouse is on left, tilts right when mouse is on right, tilts up/down based on vertical position) with Framer Motion spring physics (`stiffness: 220, damping: 22`) and dynamic cursor specular spotlight reflections (`radial-gradient`).
