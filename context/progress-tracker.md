...
- [x] Mouse Tracking Spotlight & Card-Matched Hover Lighting Restoration: Restored `z-20 pointer-events-none` spotlight overlays in `DynamicVectorCard.tsx` above card surfaces with card-matched 16% Light Mode halo (`getLightHalo()`) and 45% border ring illumination (`getBorderRing()`), and restored 8% opacity viewport ambient orb in `MouseTrackingCanvas.tsx`, guaranteeing 100% visible, vibrant mouse tracking light and click-through functionality across all cards (`src/components/DynamicVectorCard.tsx`, `src/components/MouseTrackingCanvas.tsx`)
