...
| **`GeminiImagenBannerGenerator`** | Media / Automation / AI | [`automations/shared/imagen_generator.py`](file:///c:/Users/Naresh%20Samal/Downloads/OdishaExamPrep%20Website/automations/shared/imagen_generator.py) | 2-Stage Intelligent Art Director (Gemini 3.8 Flash prompt reasoning + Gemini 3.1/2.5 Flash Image native generation with automatic official vector board fallback via `exam_logo_registry.py`). Completely eliminates generic/mismatched stock photos across all blog and social publishing | `exam_update_engine.py`, `ca_website_publisher.py`, `seo_blog_engine.py`, `drive_image_sanitizer.py` | Active |
...
- **Library Tab Academic Vector Canvas & Floating Library Watermarks (`src/App.tsx`)**: Wraps the Library tab (`PurchasesView`) in the site-wide isolated vector canvas (`bg-[radial-gradient(#cbd5e1_1.2px,transparent_1.2px)] [background-size:20px_20px]`), dual HSL ambient glow spheres (`from-brand-300/20 via-indigo-200/15`), stacking context isolation (`style={{ isolation: 'isolate' }}`), and bespoke floating academic library watermarks (`BookMarked`, `BookOpen`, `Layers`, `Award` at `opacity-[0.08] dark:opacity-[0.04]`), guaranteeing unified visual consistency across all website tabs.
- **AI Mentor Academic Vector Canvas & Themed AI Watermarks (`src/pages/AiMentor.tsx`)**: Wraps the AI Mentor workspace in the site-wide isolated vector canvas (`bg-[radial-gradient(#cbd5e1_1.2px,transparent_1.2px)] [background-size:20px_20px]`), dual HSL ambient glow spheres (`from-brand-300/20 via-indigo-200/15`), stacking context isolation (`style={{ isolation: 'isolate' }}`), and intelligent AI study vector watermarks (`Sparkles`, `Cpu`, `Brain`, `Target` at `opacity-[0.08] dark:opacity-[0.04]`), establishing complete visual harmony across the entire platform.
- **Dynamic Bi-Directional 3D Vector Card Hover & Theme-Aware Spotlight (`DynamicVectorCard.tsx`)**: High-performance 0ms latency cursor tracking engine utilizing `requestAnimationFrame` VSYNC synchronization to dynamically tilt cards bi-directionally without CSS transition delay during active cursor movement. Caches untransformed element bounds on entry to eliminate 3D projection feedback jitter, and applies a buttery-smooth `0.4s cubic-bezier(0.23, 1, 0.32, 1)` spring return upon mouse exit. Renders a smooth, professional 3-stop ambient gradient (`core → mid-ring → transparent edge`) at `z-0` behind `<div className="relative z-10">{children}</div>`, guaranteeing 100% crisp text readability, zero lag, and instant cursor synchronization across 60Hz, 120Hz, and 144Hz displays.
