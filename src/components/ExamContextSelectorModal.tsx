      ...
      <div className="fixed inset-0 z-[99999] overflow-hidden flex items-end sm:items-center justify-center p-0 sm:p-4 pointer-events-none">
        ...
        >
          ...
          <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-slate-100/90 shrink-0">
            {/* Header Title Row */}
            <div className="p-3.5 sm:p-6 pb-2.5 sm:pb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-brand-50 border border-brand-100 flex items-center justify-center text-brand-600 shrink-0 shadow-inner">
                  <Target className="w-4 h-4 sm:w-5 sm:h-5 text-brand-600" />
                </div>
                <div className="min-w-0">
                  <h3 className="text-xs sm:text-lg font-black text-slate-900 tracking-tight leading-snug truncate">
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-800 flex items-center justify-center transition-all shrink-0 active:scale-95 cursor-pointer border-none"
              >
            ...
            {/* Quick Search Bar */}
            <div className="px-3.5 sm:px-6 pb-3 sm:pb-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 sm:pl-10 pr-4 py-2 sm:py-2.5 bg-slate-50 hover:bg-white focus:bg-white border border-slate-200/90 rounded-xl text-xs sm:text-sm font-semibold text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all shadow-2xs"
                />
                {searchQuery && (
                  <button
                    type="button"
            ...
            <div className="p-3.5 sm:p-6 overflow-y-auto overscroll-contain space-y-4 sm:space-y-5 flex-1 premium-scrollbar max-h-[55vh] sm:max-h-[60vh]">
              ...
              <div>
                <button
                  ...
                  className={`w-full p-3 sm:p-4 rounded-2xl border text-left transition-all duration-200 flex items-center justify-between gap-3 cursor-pointer hover:scale-[1.008] ${
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                        <span className="text-xs sm:text-sm font-black text-slate-900 truncate">All Exams Combined</span>
                        <span className="self-start sm:self-auto px-2 py-0.5 rounded-full text-[8.5px] sm:text-[9px] font-black uppercase tracking-wider bg-brand-100 text-brand-700 border border-brand-200/60 shrink-0">
                          Aggregated View
                        </span>
                      </div>
                      <p className="text-[10px] sm:text-xs font-semibold text-slate-500 mt-0.5 leading-snug truncate">
                  ...
                  {context.activeExamId === 'all' ? (
                    <CheckCircle2 className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-brand-600 shrink-0" />
                  ) : (
                            ...
                            {cat.exams.map(exam => {
                              ...
                              return (
                                <button
                                  ...
                                  key={exam.id}
                                  onClick={() => handleSelectExam(exam.id, exam.name)}
                                  className={`w-full p-2.5 sm:p-3 rounded-xl flex items-center justify-between gap-2.5 text-left transition-all cursor-pointer hover:scale-[1.005] ${
                                    isSelected
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2 shrink-0">
                                    {isSelected ? (
                                      <>
                                        <span className="hidden sm:inline-flex px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-brand-600 text-white shadow-2xs items-center gap-1">
                                          <CheckCircle2 className="w-3 h-3 text-white" />
                                          Active Target
                                        </span>
                                        <CheckCircle2 className="sm:hidden w-4 h-4 text-brand-600 shrink-0" />
                                      </>
                                    ) : (
            ...
            <div className="sticky bottom-0 z-20 bg-slate-50/95 backdrop-blur-md border-t border-slate-200/80 px-3.5 sm:px-6 py-2.5 sm:py-3.5 flex items-center justify-between gap-3 text-xs font-semibold text-slate-500 shrink-0">
              <div className="flex items-center gap-2 min-w-0 flex-1">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shrink-0" />
                <span className="truncate text-[10px] sm:text-xs">
                  Active: <strong className="text-slate-900 font-bold">{context.activeExamName}</strong>
                </span>
              </div>
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-1.5 sm:px-5 sm:py-2 rounded-xl bg-slate-900 hover:bg-brand-600 text-white font-black text-xs uppercase tracking-wider transition-all duration-200 shadow-2xs hover:shadow-md active:scale-95 shrink-0 cursor-pointer border-none"
              >
                Close
              </button>
            </div>
