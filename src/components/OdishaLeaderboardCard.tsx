    ...
    <DynamicVectorCard glowColor="rgba(245, 158, 11, 0.15)" className="mb-6 sm:mb-8">
      ...
      <div className="space-y-2 pt-1 relative z-10">
        <div className="flex flex-wrap items-center justify-between gap-2">
          {/* Time Reset Tabs */}
          <div className="flex items-center bg-slate-800/90 p-0.5 sm:p-1 rounded-xl border border-slate-700/80 shrink-0">
            <button
              type="button"
          ...
          <div className="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-[10px] sm:text-xs font-black text-white bg-slate-800/90 border border-slate-700/90 shrink-0 font-mono shadow-2xs">
        ...
        <p className="text-xs font-semibold text-amber-200/90 pl-0.5 leading-snug">
      ...
      {podium.length >= 3 && (
        <div className="grid grid-cols-3 gap-1.5 sm:gap-4 pt-1 pb-1 items-end text-center relative z-10">
          {/* 2nd Place (Silver) */}
          <div className="p-2 sm:p-4 rounded-xl sm:rounded-2xl bg-slate-800/90 border border-slate-700/80 space-y-0.5 relative">
            ...
            <span className="font-black text-white text-[11px] sm:text-sm block pt-0.5 leading-tight">
            ...
            <span className="text-[9px] sm:text-[10px] text-slate-300 font-medium block truncate">{podium[1].district.split(' ')[0]}</span>
            <span className="inline-block px-1.5 py-0.5 rounded-md bg-slate-950 text-slate-100 font-mono font-black text-[10px] sm:text-xs border border-slate-700">
              {podium[1].xp.toLocaleString()}
            </span>
          </div>

          {/* 1st Place (Gold Podium - Elevated) */}
          <div className="p-2.5 sm:p-5 rounded-xl sm:rounded-2xl bg-gradient-to-b from-amber-500/20 via-amber-600/10 to-slate-900 border-2 border-amber-400 space-y-0.5 relative -mt-2 shadow-lg shadow-amber-500/10">
            ...
            <span className="text-[9px] sm:text-[10px] text-amber-300 font-bold block truncate">{podium[0].district.split(' ')[0]}</span>
          ...
          <div className="p-2 sm:p-4 rounded-xl sm:rounded-2xl bg-slate-800/90 border border-slate-700/80 space-y-0.5 relative">
            ...
            <span className="text-[9px] sm:text-[10px] text-slate-300 font-medium block truncate">{podium[2].district.split(' ')[0]}</span>
            <span className="inline-block px-1.5 py-0.5 rounded-md bg-slate-950 text-amber-300 font-mono font-black text-[10px] sm:text-xs border border-slate-700">
      ...
      <div className="space-y-2 pt-1 relative z-10">
        <h4 className="text-[10px] sm:text-xs font-black text-amber-300 uppercase tracking-wider pl-0.5">
        ...
        {rankList.map((entry) => (
          <div
            ...
            className={`p-2.5 sm:p-3 rounded-xl border flex items-center justify-between text-xs transition-all ${
              entry.isCurrentUser
                ? 'bg-amber-400/20 border-amber-400/60 text-white font-bold ring-1 ring-amber-400/30'
                : 'bg-slate-800/70 border-slate-700/70 hover:bg-slate-800 text-white'
            }`}
          >
            <div className="flex items-center gap-3 sm:gap-3.5 min-w-0">
              <span className="w-7 sm:w-8 font-mono font-black text-amber-400 text-left shrink-0 text-[11px] sm:text-xs">#{entry.rank}</span>
              ...
              <div className="min-w-0 pr-1">
                <span className="font-bold text-white block truncate text-xs">{entry.name}</span>
                <span className="text-[9px] sm:text-[10px] text-slate-300 font-medium block truncate">
            ...
            <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg bg-slate-900 border border-slate-700 text-amber-300 font-mono font-black text-[10px] sm:text-xs shrink-0 shadow-2xs">
      ...
      {userEntry.rank > 10 && nearbyBracket.length > 0 && (
        <div className="space-y-1.5 pt-2.5 border-t border-slate-700/80 relative z-10">
          <div className="flex items-center justify-between pl-0.5">
            <h4 className="text-[10px] sm:text-xs font-black text-amber-300 uppercase tracking-wider flex items-center gap-1">
            ...
            <span className="text-[9px] text-amber-200/90 font-bold hidden sm:inline">Overtake +20 XP to jump rank!</span>
          </div>
          ...
          {nearbyBracket.map((entry) => (
            <div
              ...
              className={`p-2.5 sm:p-3 rounded-xl border flex items-center justify-between text-xs transition-all ${
                entry.isCurrentUser
                  ? 'bg-amber-400/25 border-amber-400/70 text-white font-black'
                  : 'bg-slate-800/70 border-slate-700/70 text-white'
              }`}
            >
              <div className="flex items-center gap-3 sm:gap-3.5 min-w-0">
                <span className={`w-14 sm:w-16 font-mono font-black text-left shrink-0 text-[10px] sm:text-xs ${entry.isCurrentUser ? 'text-amber-300 text-xs sm:text-sm font-extrabold' : 'text-amber-400'}`}>
                ...
                <div className="min-w-0 pr-1">
                  <span className={`font-bold block truncate text-xs ${entry.isCurrentUser ? 'text-white font-black' : 'text-white'}`}>
              ...
              <span className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg border font-mono font-black text-[10px] sm:text-xs shrink-0 ${
                entry.isCurrentUser ? 'bg-amber-400 text-slate-950 border-amber-300 shadow-xs' : 'bg-slate-900 text-amber-300 border-slate-700'
              }`}>
                {entry.xp.toLocaleString()} XP
              </span>
            </div>
