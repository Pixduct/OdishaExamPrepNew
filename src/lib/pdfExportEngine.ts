...
function renderMathForPdf(text: string): string {
  ...
  return DOMPurify.sanitize(processed, {
    ADD_TAGS: ['span', 'div', 'p', 'br', 'strong', 'em', 'sub', 'sup', 'table', 'tr', 'td', 'th', 'tbody', 'thead', 'svg', 'path'],
    ADD_ATTR: ['class', 'style', 'aria-hidden', 'viewBox', 'd', 'fill', 'stroke', 'stroke-width', 'stroke-linecap', 'stroke-linejoin']
  });
}
...
/**
 * Exports and prints a Question Bank directly to PDF via styled browser print window
 */
export function exportQuestionBankToPdf(data: QuestionBankExportData): void {
  ...
  // Temporarily set the active browser tab title so Chromium auto-fills the Save dialog filename
  ...
  // Use hidden iframe to trigger print / PDF save
