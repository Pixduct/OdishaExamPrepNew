...
export const OdishaLeaderboardCard: React.FC<OdishaLeaderboardCardProps> = ({ userId }) => {
  ...
  return (
    <DynamicVectorCard glowColor="rgba(245, 158, 11, 0.15)" className="mb-6 sm:mb-8">
      <div className="p-5 sm:p-7 text-slate-900 dark:text-white rounded-[2.2rem] bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none space-y-4 sm:space-y-5 relative overflow-hidden">
      ...
      <div className="space-y-2 pt-1 relative z-10">
        <div className="flex flex-wrap items-center justify-between gap-2">
          {/* Time Reset Tabs */}
          <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 sm:p-1 rounded-xl border border-slate-200/80 dark:border-slate-700/80 shrink-0">
            <button
              type="button"
      ...
      {podium.length >= 3 && (
        <div className="grid grid-cols-3 gap-1.5 sm:gap-4 pt-1 pb-1 items-end text-center relative z-10">
          {/* 2nd Place (Silver) */}
          <div className="p-2 sm:p-4 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/80 space-y-0.5 relative">
          ...
          <div className="p-2.5 sm:p-5 rounded-xl sm:rounded-2xl bg-gradient-to-b from-amber-100/80 via-amber-50/50 to-white dark:from-amber-500/20 dark:via-amber-600/10 dark:to-slate-900 border-2 border-amber-400 space-y-0.5 relative -mt-2 shadow-lg shadow-amber-500/10">
        ...
        {rankList.map((entry) => (
          <div
            ...
            className={`p-2.5 sm:p-3 rounded-xl border flex items-center justify-between text-xs transition-all ${
              entry.isCurrentUser
                ? 'bg-amber-50 dark:bg-amber-400/20 border-amber-300 dark:border-amber-400/60 text-slate-900 dark:text-white font-bold ring-1 ring-amber-400/30'
                : 'bg-slate-50/80 dark:bg-slate-800/70 border-slate-200/80 dark:border-slate-700/70 hover:bg-slate-100/80 dark:hover:bg-slate-800 text-slate-900 dark:text-white'
            }`}
          >
      ...
      {userEntry.rank > 10 && nearbyBracket.length > 0 && (
        <div className="space-y-1.5 pt-2.5 border-t border-slate-200/80 dark:border-slate-700/80 relative z-10">
          ...
          {nearbyBracket.map((entry) => (
            <div
              ...
              className={`p-2.5 sm:p-3 rounded-xl border flex items-center justify-between text-xs transition-all ${
                entry.isCurrentUser
                  ? 'bg-amber-100 dark:bg-amber-400/25 border-amber-300 dark:border-amber-400/70 text-slate-900 dark:text-white font-black'
                  : 'bg-white dark:bg-slate-800/70 border-slate-200 dark:border-slate-700/70 text-slate-900 dark:text-white'
              }`}
            >
              <div className="flex items-center gap-3 sm:gap-3.5 min-w-0">
