...
export default function YouTubeCarousel({ videoIds }: { videoIds?: string[] }) {
  ...
  return (
    ...
    >
    <div ref={containerRef} className="w-full relative py-3 sm:py-12 overflow-hidden bg-[#F2EFE9]/90 dark:bg-slate-900/90 backdrop-blur-md border-2 border-slate-900 dark:border-slate-700 rounded-[2rem] sm:rounded-[2.5rem] shadow-[6px_6px_0px_rgba(0,0,0,1)] sm:shadow-[8px_8px_0px_rgba(0,0,0,1)] select-none">
        ...
        >
          {items.map((video, idx) => {
            ...
            return (
              ...
              <div
                className={cn(
                  "relative w-full h-full shrink-0 rounded-2xl overflow-hidden border-2 border-slate-900 dark:border-slate-700 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md group/video transition-all cursor-pointer flex flex-col",
                  isMobile
